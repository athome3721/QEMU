...
static void *ls1b_intctl_init(hwaddr addr,qemu_irq parent_irq);
...
static void mips_ls1b_init (QEMUMachineInitArgs *args)
{
   ...
   sysbus_create_simple("ls1gp_copdma",0x1c000000, ls1b_irq1[10]);


}

QEMUMachine mips_ls1gp_machine = {
	.name = "ls1gp",
...
{
	qemu_register_machine(&mips_ls1gp_machine);
}
...
static void *ls1b_intctl_init(hwaddr addr,qemu_irq parent_irq)
{
	...
	{
                MemoryRegion *iomem = g_new(MemoryRegion, 1);
                memory_region_init_io(iomem, &ls1b_intctl_mem_ops, s, "ls1b_int", INTCTL_SIZE);
