...
{
    ls1a_wdt_state *d = FROM_SYSBUS(ls1a_wdt_state, dev);
#ifdef DEBUG_LS1F_WDT
	select_watchdog_action("debug");
#endif

    memory_region_init_io(&d->iomem, &ls1a_wdt_ops, (void *)d, "ls1a wdt", 0xc);

    sysbus_init_mmio(dev, &d->iomem);
    d->timer = qemu_new_timer_ms(vm_clock, ls1a_wdt_timer_expired, d);
