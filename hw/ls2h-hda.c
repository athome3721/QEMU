...
static void intel_hda_response(HDACodecDevice *dev, bool solicited, uint32_t response)
{
    ...
    dma_memory_write(d->dmactx, addr + 8*wp, &response, 4);
    dma_memory_write(d->dmactx, addr + 8*wp + 4, &ex, 4);
    d->rirb_wp = wp;
                           ...
                           uint8_t *buf, uint32_t len)
{
	...
	dma_memory_rw(d->dmactx, st->bpl[st->be].addr + st->bp, buf, copy, !output);
        st->lpib += copy;
        st->bp += copy;
        buf += copy;
        left -= copy;
	...
	dma_memory_write(d->dmactx, addr + 8*s, &st->lpib, 4);
    }
    dprint(d, 3, "dma: --\n");
...
static void intel_hda_parse_bdl(IntelHDAState *d, IntelHDAStream *st)
{
	...
	dma_memory_read(d->dmactx, addr, buf, 16);
...
{
    BusChild *kid;
    IntelHDAState *d = DO_UPCAST(IntelHDAState, busdev.qdev, dev);
    HDACodecDevice *cdev;
...
static int intel_hda_init(SysBusDevice *dev)
{
    IntelHDAState *d = DO_UPCAST(IntelHDAState, busdev, dev);
    ...
    if(!d->dmactx) d->dmactx = &dma_context_memory;

    hda_codec_bus_init(&d->busdev.qdev, &d->codecs,
 ...
 */
int intel_hda_and_codec_init(SysBusDevice *dev)
{
