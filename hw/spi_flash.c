...
static void program_page(SPIFlashState *s, int addr, unsigned char value, int count)
{
	...
	if(s->bdrv)
	{
		bdrv_read(s->bdrv, addr>>9, pp_buf, 1);
		pp_buf[addr&0x1ff] = value;
		bdrv_write(s->bdrv, addr>>9, pp_buf, 1);
	}
}
...
static void sector_erase(SPIFlashState *s, int addr)
{
	...
	if(s->bdrv)
		bdrv_write(s->bdrv, sec_addr >> 9, buf, 128);
	g_free(buf);
}
...
static Property spi_flash_properties[] = {
    ...
    DEFINE_PROP_DRIVE("drive", SPIFlashState, bdrv),
    DEFINE_PROP_END_OF_LIST(),
};



static int spi_flash_init(SSISlave *dev)
{
	SPIFlashState *s = FROM_SSI_SLAVE(SPIFlashState, dev);
	int len;

	s->mode = SPI_FLASH_CMD;
	memory_region_init_rom_device(&s->mem, &spi_rom_ops , s, "spi flash rom", 0x800000);
  	s->buf = memory_region_get_ram_ptr(&s->mem);

        memory_region_init_alias(&s->mem1, "spi bios", &s->mem, 0, s->size);

	if(s->bdrv && s->bdrv->filename[0])
	{
		len = bdrv_getlength(s->bdrv);
		if(len != 0x800000) {
			printf("length=%d must be 8 mega bytes,run command bellow to trucate file to size:\n", len);
			printf("perl -e 'truncate qq(%s),%d'\n",(s->bdrv->filename),0x800000);
			//exit(0);
		}
		len = MIN(0x800000,len);
		bdrv_read(s->bdrv, (int64_t)0LL, s->buf, len>>9);
	}
