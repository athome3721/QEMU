...
{
    uint32_t d;
    d = muldiv64(qemu_get_clock_ms(vm_clock), PM_FREQ, get_ticks_per_sec());
    return d & 0xffffff;
}
    ...
    case 0x00:
        {
            ...
            if (pmsts & val & TMROF_EN) {
                ...
                d = muldiv64(qemu_get_clock_ms(vm_clock), PM_FREQ,
                             get_ticks_per_sec());
...
{
    PIIX4PMState *d = FROM_SYSBUS(PIIX4PMState, dev);

    memory_region_init_io(&d->iomem, &pm_ops, (void *)d, "ls1a_acpi", 0x1000);

    sysbus_init_mmio(dev, &d->iomem);
    sysbus_init_irq(dev, &d->irq);
    d->tmr_timer = qemu_new_timer_ms(vm_clock, pm_tmr_timer, d);
