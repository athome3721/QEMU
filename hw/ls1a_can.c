...
static void ls1a_can_recv_timer(void *vp)
{
	...
	qemu_mod_timer(s->timer, qemu_get_clock_ms (vm_clock) + timeout);
}
...
{
    ls1a_can_state *d = FROM_SYSBUS(ls1a_can_state, dev);

    memory_region_init_io(&d->iomem, &ls1a_can_ops, (void *)d, "ls1a i2c", 0x4000);
    ls1a_can_reset(d);
	...
	ls1a_can_data.sendfrom=255;
        d->timer = qemu_new_timer_ms(vm_clock, ls1a_can_recv_timer, d);
	int64_t timeout = (int64_t) SCALE_MS>>4;
	qemu_mod_timer(d->timer, qemu_get_clock_ms (vm_clock) + timeout);
